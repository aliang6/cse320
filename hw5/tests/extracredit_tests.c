...
Test(map_suite, 04_putdel_2, .init = map_init, .fini = map_fini) {
    pthread_t thread_ids[NUM_THREADS];
    pthread_t thread_ids_2[NUM_THREADS];
    map_insert_t* inserted[NUM_THREADS];
    ...
    // wait for threads to die before checking map
    for(int index = 0; index < NUM_THREADS; index++) {
        pthread_join(thread_ids[index], NULL);
    }
    ...
    for(int index = 0; index < NUM_THREADS; index++) {
        ...
        if(pthread_create(&thread_ids_2[index], NULL, thread_del, inserted[index]) != 0)
            exit(EXIT_FAILURE);
    }
    ...
    int num_items = global_map->size;
    cr_assert_eq(num_items, 0, "Had %d items in map. Expected 0", num_items);
}